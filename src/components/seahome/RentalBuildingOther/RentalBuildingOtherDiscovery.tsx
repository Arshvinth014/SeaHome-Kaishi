...
import {
    ...
    SEARCH_TOOLS_BUILDING,
    RENTAL_BUILDING_TERMS,
    AFFILIATED_STORE_BANNER_BUILDING_DATA,
    FOOTER_DISCLAIMER_BUILDING_TEXT,
    type BuildingListingItem,
    type NamedLink,
} from './rentalBuildingOtherDiscoveryData';

interface RentalBuildingOtherDiscoveryProps {
    onNavigate?: (path: string) => void;
}

export const RentalBuildingOtherDiscovery: React.FC<RentalBuildingOtherDiscoveryProps> = ({ onNavigate }) => {
    ...
    return (
        <div className="mt-8 space-y-6 w-full font-sans">
            ...
            <section aria-label="What's New Building & Factory Listings" className="overflow-hidden rounded-xl border border-blue-100/90 bg-white p-4 sm:p-5 shadow-xs">
                <div className="mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="ml-2 rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-semibold text-blue-700 border border-blue-200/60">
                            {WHAT_S_NEW_BUILDING_LISTINGS.length} listings
                        </span>
                    </div>
                </div>
                ...
                <div className="group relative">
                    ...
                    >
                        {WHAT_S_NEW_BUILDING_LISTINGS.map((item: BuildingListingItem) => (
                            <div
                                key={item.id}
            ...
            <section className="rounded-xl border border-blue-100/80 bg-white p-4 sm:p-5 shadow-xs">
                ...
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2.5 text-xs sm:text-sm font-medium">
                    {OTHER_PROPERTY_TYPES_BUILDING.map((item: NamedLink, idx: number) => (
                        <button
                            key={idx}
                            type="button"
            ...
            <section className="rounded-xl border border-blue-100/80 bg-white p-4 sm:p-5 shadow-xs">
                ...
                <div className="flex flex-wrap items-center gap-x-8 gap-y-2.5 text-xs sm:text-sm font-medium">
                    {SEARCH_TOOLS_BUILDING.map((item: NamedLink, idx: number) => (
                        <button
                            key={idx}
                            type="button"
                            onClick={() => handleLinkClick(item.path)}
            ...
            {/* 4. RENTAL AND OTHER TERMINOLOGY & GLOSSARY */}
            <section className="rounded-xl border border-blue-100/80 bg-white p-4 sm:p-5 shadow-xs">
                ...
                <div className="mt-3.5 flex flex-wrap items-center gap-x-6 gap-y-2.5 text-xs sm:text-sm font-medium border-t border-gray-100 pt-3">
                    {RENTAL_BUILDING_TERMS.map((item: NamedLink, idx: number) => (
                        <button
                            key={idx}
                            type="button"
                            onClick={() => handleLinkClick(item.path)}
            ...
            <section
                onClick={() => handleLinkClick(AFFILIATED_STORE_BANNER_BUILDING_DATA.link)}
            ...
            >
                <div className="h-20 w-32 shrink-0 overflow-hidden rounded-lg border border-blue-100 bg-white shadow-2xs">
                    <img
                        src={AFFILIATED_STORE_BANNER_BUILDING_DATA.imageUrl}
                        alt={AFFILIATED_STORE_BANNER_BUILDING_DATA.title}
                ...
                <div className="flex-1 text-center sm:text-left">
                    <h4 className="flex items-center justify-center gap-2 text-sm font-extrabold text-blue-800 transition-colors group-hover:text-blue-900 sm:justify-start sm:text-base">
                        ...
                        <span className="underline decoration-blue-300 group-hover:no-underline">
                            {AFFILIATED_STORE_BANNER_BUILDING_DATA.title}
                        </span>
                    </h4>
                    <p className="mt-1.5 text-xs text-gray-700 sm:text-sm leading-relaxed">
                        {AFFILIATED_STORE_BANNER_BUILDING_DATA.description}
                    </p>
                </div>
            </section>
...
export default RentalBuildingOtherDiscovery;
