...
import {
    ...
    OTHER_PROPERTY_TYPES_WAREHOUSE,
    SEARCH_TOOLS_WAREHOUSE,
    NOTABLE_CITIES_WAREHOUSE,
    STATIONS_TO_WATCH_WAREHOUSE,
    AFFILIATED_STORE_BANNER_WAREHOUSE_DATA,
    FOOTER_DISCLAIMER_WAREHOUSE_TEXT,
    type WarehouseListingItem,
    type NamedLink,
} from './warehouseDiscoveryData';

interface WarehouseDiscoveryProps {
...
export const WarehouseDiscovery: React.FC<WarehouseDiscoveryProps> = ({ onNavigate }) => {
    ...
    return (
        <div className="mt-8 space-y-6 w-full font-sans">
            ...
            <section aria-label="What's New Rental Warehouse Listings" className="overflow-hidden rounded-xl border border-blue-100/90 bg-white p-4 sm:p-5 shadow-xs">
                <div className="mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="ml-2 rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-semibold text-blue-700 border border-blue-200/60">
                            {WHAT_S_NEW_WAREHOUSE_LISTINGS.length} listings
                        </span>
                    </div>
                </div>
            ...
            <section className="rounded-xl border border-blue-100/80 bg-white p-4 sm:p-5 shadow-xs">
                ...
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2.5 text-xs sm:text-sm font-medium">
                    {OTHER_PROPERTY_TYPES_WAREHOUSE.map((item: NamedLink, idx: number) => (
                        <button
                            key={idx}
                            type="button"
            ...
            {/* 4. NOTABLE CITIES AND DISTRICTS */}
            <section className="rounded-xl border border-blue-100/80 bg-white p-4 sm:p-5 shadow-xs">
                <div className="mb-3 flex items-center gap-2">
                    <span className="inline-block h-5 w-1.5 shrink-0 rounded-full bg-blue-600" />
                    <MapPin className="h-4 w-4 text-blue-600 hidden sm:inline" />
                    <h3 className="text-base font-extrabold text-gray-900 sm:text-lg">
                        Notable cities and districts
                    </h3>
                </div>

                <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs sm:text-sm font-medium">
                    {NOTABLE_CITIES_WAREHOUSE.map((item: NamedLink, idx: number) => (
                        <button
                            key={idx}
                            type="button"
                            onClick={() => handleLinkClick(item.path)}
            ...
            <section className="rounded-xl border border-blue-100/80 bg-white p-4 sm:p-5 shadow-xs">
                <div className="mb-3 flex items-center gap-2">
                    <span className="inline-block h-5 w-1.5 shrink-0 rounded-full bg-blue-600" />
                    <Train className="h-4 w-4 text-blue-600 hidden sm:inline" />
                    <h3 className="text-base font-extrabold text-gray-900 sm:text-lg">
                        Stations to watch
                    </h3>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs sm:text-sm font-medium">
                    {STATIONS_TO_WATCH_WAREHOUSE.map((item: NamedLink, idx: number) => (
                        <button
                            key={idx}
                            type="button"
                            onClick={() => handleLinkClick(item.path)}
            ...
            <section
                onClick={() => handleLinkClick(AFFILIATED_STORE_BANNER_WAREHOUSE_DATA.link)}
            ...
            >
                <div className="h-20 w-32 shrink-0 overflow-hidden rounded-lg border border-blue-100 bg-white shadow-2xs">
                    <img
                        src={AFFILIATED_STORE_BANNER_WAREHOUSE_DATA.imageUrl}
                        alt={AFFILIATED_STORE_BANNER_WAREHOUSE_DATA.title}
                ...
                <div className="flex-1 text-center sm:text-left">
                    <h4 className="flex items-center justify-center gap-2 text-sm font-extrabold text-blue-800 transition-colors group-hover:text-blue-900 sm:justify-start sm:text-base">
                        ...
                        <span className="underline decoration-blue-300 group-hover:no-underline">
                            {AFFILIATED_STORE_BANNER_WAREHOUSE_DATA.title}
                        </span>
                    </h4>
                    <p className="mt-1.5 text-xs text-gray-700 sm:text-sm leading-relaxed">
                        {AFFILIATED_STORE_BANNER_WAREHOUSE_DATA.description}
                    </p>
                </div>
            </section>
...
export default WarehouseDiscovery;
