  ...
  return (
    <div className="min-h-screen bg-slate-50/60 pb-16 text-slate-800 font-sans selection:bg-sky-500 selection:text-white">
      ...
      <div className={`${HUB_CONTAINER} pt-6 pb-10 space-y-6`}>
        {/* PAGE HERO HEADER TITLE (Ocean Blue theme matching SeaHome Net styling) */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-sky-200/80">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-100 text-sky-800 border border-sky-200 text-xs font-bold mb-2">
              <MapPin className="w-3.5 h-3.5 text-sky-600" />
              <span>Interactive Real Map Search</span>
            </div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight leading-snug">
              ...
              <span className="bg-gradient-to-r from-sky-600 to-blue-700 bg-clip-text text-transparent font-extrabold">
                {prefData.prefectureName} Prefecture
              </span>{' '}
              using a map.
            </h1>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 border-sky-600 bg-white text-sky-700 font-bold text-xs hover:bg-sky-50 transition-all shadow-xs cursor-pointer"
              >
              ...
              <button
                type="button"
                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gradient-to-r from-sky-600 to-blue-700 text-white font-bold text-xs shadow-sm ring-2 ring-sky-500/30 cursor-default"
              >
        ...
        <div className="pt-2">
          <h2 className="text-sm font-bold text-slate-800 flex items-center gap-2">
            ...
            Please select a city/ward/county or click a map tag.
          </h2>
        </div>

        {/* REAL MAP VISUALIZATION CARD */}
        ...
        {/* CITY / DISTRICT BREAKDOWN LIST SECTION */}
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="bg-sky-50/90 border-b border-sky-100 px-5 py-3.5">
            <h3 className="text-sm font-extrabold text-sky-950 flex items-center gap-2">
              <span className="w-2 h-4 rounded-full bg-sky-600" />
                  ...
                  >
                    <span
                      className={`font-semibold group-hover:underline underline-offset-2 ${
                        hasListings ? 'text-sky-700 font-bold group-hover:text-blue-900' : 'text-slate-400'
                      }`}
                    >
                      {city.name}
                    </span>
                    <span
                      className={`text-[11px] ${
                        hasListings ? 'text-slate-500 font-medium' : 'text-slate-300'
                      }`}
                    >
                      ({city.count})
                    </span>
